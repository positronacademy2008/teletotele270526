...
print("🛠 [DEBUG] SYSTEM BOOTING UP WITH ADVANCED MIRRORING & BRAND PROTECTION...")
...
# 🔥 ISSUE 2 FIX: Aggressive Brand Replacer (Replaces underlying hrefs too)
def brand_replacer(text: str) -> str:
    # 1. Replace Competitor Names
    ...
    # 3. Replace ANY Telegram/WhatsApp URLs (Direct text & HTML attributes)
...
# --- ISSUE 1 FIX: STRICT XML RSS PARSER ---
...
def publish_to_wordpress(title, content):
    ...
    # Run final aggressive brand replacer on generated HTML
    ...
    clean_slug = f"update-{int(time.time() * 1000)}" # Unique slug
    data = {'title': brand_replacer(title), 'content': final_clean_content, 'status': 'publish', 'slug': clean_slug}
...
# --- ISSUE 3 FIX: DEEP SCRAPER & SUB-POST MIRRORING ---
def deep_scrape_and_mirror(url):
    ...
    try:
        ...
        # Make a quick sub-post on WP
        print(f"   ↳ ⏳ [DEBUG] Creating sub-post for mirrored content...")
        mirrored_link = publish_to_wordpress(f"Details: {page_title[:40]}", page_content)
    ...
    return url # Fallback original if mirroring fails
...
def main():
    ...
    for current_item in new_items:
        ...
        # Attachment Dropper for Competitor Name
        if "राजस्थान न्यूज़ टूडे" in raw_text: # Name is already replaced above
            print("   ↳ 🛠 [DEBUG] Competitor keyword found. Dropping PDF/Image.")
            current_item["enclosure_url"] = None
            ctype = "" 

        # Scan for links in the message
        ...
        if found_links:
            ...
            # ISSUE 3 FIX: Intercept Indianaukrihelp right here
            if "indianaukrihelp.com" in primary_link:
                new_mirrored_link = deep_scrape_and_mirror(primary_link)
                # Replace in the raw text so AI gets the new WP link
                raw_text = raw_text.replace(primary_link, new_mirrored_link)
                webpage_scraped_data = raw_text
                primary_link = new_mirrored_link # Update for further processing
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        for a in soup.find_all('a', href=True):
                            ...
                            # Deep Scraper (Mirroring links found INSIDE the scraped page)
                        ...
                        page_text = brand_replacer(page_text) # Apply brand protection to scraped data
